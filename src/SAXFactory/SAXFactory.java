...
//    private static final String LOGGING_LEVEL = "SEVERE";
    ...
    public static DiscordRecords ts2Discords(double[] tsData, int windowSize, int alphabetSize)
        ...
        int[] totalDistFuncCnt = new int[]{0};
        DiscordRecords discords = getDiscordsAlgorithm(tsData, windowSize, trie,
                DEFAULT_COLLECTION_SIZE, new LargeWindowAlgorithm(), totalDistFuncCnt);
     ...
     * @throws TSException if error occurs.
     */
    public static DiscordRecords instances2Discords(double[] series,
            int windowSize, int alphabetSize, int discordsNumToReport) throws TrieException, TSException {
        ...
        int[] totalDistFuncCnt = new int[]{0};
        DiscordRecords discords = getDiscordsAlgorithm(series, windowSize,
                trie, reportNum, new LargeWindowAlgorithm(), totalDistFuncCnt);
        Date end = new Date();
        consoleLogger.info("discords search finished in : "
                + timeToString(start.getTime(), end.getTime()) + ", #distfunc: " + totalDistFuncCnt[0]);
    ...
    public static DiscordRecords series2Discords(double[] series, int windowSize, int alphabetSize,
        ...
        int[] totalDistFuncCnt = new int[]{0};
        DiscordRecords discords = getDiscordsAlgorithm(series, windowSize, trie, reportNum,
                markerAlgorithm, totalDistFuncCnt);
        Date end = new Date();
        consoleLogger.fine("discords search finished in : "
     ...
     *
     * Here I need to keep a continuous stack of knowledge with information not
     * only about distance, but about abandoning or conducting a full search for
     * words. Thus, I will not be doing the same expensive search on the rarest
     * word all over again.
     *
    ...
    private static DiscordRecords getDiscordsAlgorithm(double[] series, int windowSize, SAXTrie trie,
            int discordCollectionSize, SlidingWindowMarkerAlgorithm marker, int[] totalDistanceFunctionCounter) throws TSException,
            TrieException {
        ...
        while (discords.getSize() < discordCollectionSize) {
            ...
            consoleLogger.fine("currently known discords: " + discords.getSize() + " out of "
            ...
            DiscordRecord bestDiscord = findBestDiscord(series, windowSize, trie, completeWords,
                    knownWordsAndTheirCurrentDistances, discordsVisitRegistry, marker, distFuncCnt);
            Date end = new Date();
            ...
            consoleLogger.info("new discord: " + bestDiscord.getPayload() + ", position "
                    + bestDiscord.getPosition() + ", distance " + bestDiscord.getDistance()
                    + ", elapsed time: " + timeToString(start.getTime(), end.getTime()) + ", #distfunc: " + distFuncCnt[0]);

            totalDistanceFunctionCounter[0] = totalDistanceFunctionCounter[0] + distFuncCnt[0];
            // collect the result
            //
     ...
     * approximately the best. Due to the fuzzy-logic search with randomization
     * and aggressive labeling of the magic array locations.
     *
     * @param series The series we are looking for discord in.
     * @param windowSize The sliding window size.
     * @param trie The trie (index of the series).
     * @param foundDiscordsWords Already found discords.
     * @param knownWordsAndTheirDistances The best known distances for certain
     * word. I use the early search abandoning optimization in oder to reduce
     * complexity.
     ...
     * @throws TrieException If error occurs.
     */
    public static DiscordRecord findBestDiscord(double[] series, int windowSize, SAXTrie trie,
            TreeSet<String> foundDiscordsWords,
            TreeMap<String, DistanceEntry> knownWordsAndTheirDistances, VisitRegistry visitedLocations,
            SlidingWindowMarkerAlgorithm marker, int[] distanceFunctionCounter) throws TSException, TrieException {

        // Counting the number of invokes of distance function
        distanceFunctionCounter[0] = 0;
    ...
    // StringBuilder sb = new StringBuilder();
        ...
        while (idx < limit) {
            ...
            if (foundDiscordsWords.contains(currentWord) || visitedLocations.isVisited(currentPosition)) {
                ...
                consoleLogger.finer("conducting search for " + currentWord + " iteration " + idx + " from "
                        + frequencies.size());
            }
            ...
            double[] currentLocations = getSubSeries(series, currentPosition, currentPosition
                    + windowSize);
            marker.markVisited(registry, currentPosition, windowSize);

            // WE QRE GOING TO ITERATE OVER THE CURRENT WORD OCCURENCES HERE
            //
            DistanceEntry bestKnownDistance = knownWordsAndTheirDistances.get(String.valueOf(currentEntry
                    .getStr()));

            if (null != bestKnownDistance && !(bestKnownDistance.isAbandoned())) {
                consoleLogger.finer("skipping iterations over " + currentWord
                        + " retrieved result from known distances ");
                ...
                consoleLogger.finer(currentWord + " has " + currentOccurences.size()
                ...
                for (Integer nextOccurrence : currentOccurences) {
                    ...
                    double[] occurrenceValues = getSubSeries(series, nextOccurrence, nextOccurrence
                            + windowSize);
                    double dist = ED.distance(currentLocations, occurrenceValues);
            ...
            if (!(Double.MAX_VALUE == nearestNeighborDist)) {
                consoleLogger.finer("for " + currentWord
            ...
            if (doRandomSearch) {
                ...
                while ((nextRandomVisitTarget = registry.getNextRandomUnvisitedPosition()) != -1) {
                    ...
                    double randomTargetDistance = ED.distance(currentLocations,
                            randomTargetValues);
                    distanceFunctionCounter[0]++;
                    ...
                    if (randomTargetDistance < bestSoFarDistance) {
                        ...
                        consoleLogger.finer(" ** abandoning random visits loop, seen distance "
                ...
                consoleLogger.finer("random visits loop finished, total positions considered: "
                        + visitCounter);
        ...
        while ((currPosition + window) < tsData.size()) {
            ...
            for (int i = 0; i < controls.length; i++) {
                if (Math.abs(controls[i] - currPosition) < window) {
                    ...
                    double dist = ED.distance(vals, oVals);
                    if (distances[i] > dist) {
                        distances[i] = dist;
    ...
    /**
     * // * // * "We are given n, the length of the discords in advance, and we
     * must // choose two parameters, // the // * cardinality of the SAX
     * alphabet size a, and the SAX word size w. We // defer a discussion of how
     * // * to set these parameters until" // * // * // * @param tsData
     * timeseries. // * @param windowLength window length. // * @param paaSize
     * The PAA window size. // * @param alphabetSize The SAX alphabet size. //
     *
     *
     * @param timeAttributeName Time-stamp attribute. // * @param
     * dataAttributeName Value attribute. // * @return top discords for the
     * time-series given // * @throws TSException if error occurs. //
     */
    ...
    public static DiscordRecords getBruteForceDiscords(double[] tsData, int windowLength)
        ...
        for (int i = 0; i < tsData.length - windowLength; i++) {
            ...
            for (int j = 0; j < tsData.length - windowLength; j++) {
                ...
                double dist = ED.distance(seriesA, seriesB);
    ...
    public static DiscordRecords getBruteForceDiscords(double[] tsData, int windowLength, int reportNum)
            throws TSException {

        DiscordRecords discords = new DiscordRecords();
        SlidingWindowMarkerAlgorithm marker = new LargeWindowAlgorithm();
        VisitRegistry discordsVisitRegistry = new VisitRegistry(tsData.length - windowLength);
        while (discords.getSize() < reportNum) {

            DiscordRecord bestDiscord = getBestBruteForceDiscord(tsData, windowLength, discordsVisitRegistry);
            if (bestDiscord.getDistance() == 0.0D || bestDiscord.getPosition() == -1) {
                return discords;
            }
            discords.add(bestDiscord);
            marker.markVisited(discordsVisitRegistry, bestDiscord.getPosition(), windowLength);
            consoleLogger.info("Find #" + discords.getSize() + " discord: " + bestDiscord.getPayload() + " at "
                    + bestDiscord.getPosition() + ", distance " + bestDiscord.getDistance());
        }
        // run the search loop
        //
    ...
    public static DiscordRecord getBestBruteForceDiscord(double[] tsData, int windowLength, VisitRegistry discordsVisitRegistry) throws TSException {
        int bestSoFarPosition = -1;
        double bestSoFarDistance = 0.0D;
        for (int i = 0; i < tsData.length - windowLength; i++) {

            double nndist = Double.MAX_VALUE;
            if (discordsVisitRegistry.isVisited(i)) {
                continue;
            }
            double[] seriesA = getSubSeries(tsData, i, i + windowLength);
        ...
        return (new DiscordRecord(bestSoFarPosition, bestSoFarDistance));
    }
